...
vec3 Scene::raytrace( vec3 &rayStart, vec3 &rayDir, int depth, int thisObjIndex, int thisObjPartIndex, float accumulatedWeight )

{
  ...
  // survives, give it more weight.
    ...
    //Initial accumulated weight 
    float accumulatedW;
    if (depth == 0) {
        accumulatedW = 1;
    }
    else {
        accumulatedW = accumulatedWeight;
    }

	//CALCULATE WEIGHT OF RAY
    if (thisObjIndex > -1) {
        //Specular coefficient chain
        Object& object = *objects[thisObjIndex];
        //Multiply the next specular coefficient for this bounce. Only need one 
        accumulatedW = accumulatedW * object.mat->ks.x;
    }
    ...
    float actualW;
    float weightThreshold = 0.126;
    float probability = 0.5;
    bool survivedRoulette = false;

    //At bounce 
    if (accumulatedW >= weightThreshold) {
        //Ray continues
        actualW = accumulatedW;
    }
    else {
        //Perform russian roulette to roll the dice
        if (randIn01() <= probability) {
            //Ray terminated. Results in black color
            actualW = 0;
            return actualW * rayStart;
        }
        else {
            //Ray survives and is given more weight
            survivedRoulette = true;
        }
    }
#endif
  ...
  vec3 Iin = raytrace( P, R, depth, objIndex, objPartIndex, accumulatedW);
  ...
  for (int i=0; i<objects.size(); i++) {
    ...
    float pointAmount = 50;
      for(int numPoints = 0; numPoints < pointAmount; numPoints++) {
        float alpha = randIn01();
        float beta = randIn01();
        if (alpha + beta <= 1){
          vec3 randPoint = v0 + (alpha * a) + (beta * b);
          vec3 L1 = randPoint - P; 
          if (N*L1 > 0){

            float L1Dist = L1.length();
            L1 = (1.0/L1Dist) * L1;
            ...
            bool found = findFirstObjectInt( P, L1, objIndex, objPartIndex, intP, intN, intTexCoords, intT, intObjIndex, intObjPartIndex, intMat, -1 );
            if (!found || intT > L1Dist) { // no object: Add contribution from this light
            vec3 Lr = (2 * (L1 * N)) * N - L1;
            Iout = Iout + (1.0 / pointAmount) * calcIout( N, L1, E, Lr, kd, mat->ks, mat->n, tri->mat->Ie);
            }
  ...
  if (survivedRoulette) {
      return (1.0 / (1.0 - probability)) * Iout;
  }
  return Iout;
}
...

{
    ...
    //Divide pixel into numPixelSamples x numPixelSamples grid
    float divisionAmount = numPixelSamples * numPixelSamples;
    for (int i = 0; i < divisionAmount; i++) {
            if (jitter) {
                //Sum results with position between 0 and 1
                vec3 dir = (llCorner + (x + randIn01()) * right + (y + randIn01()) * up).normalize();
                total = total + raytrace(eye->position, dir, 0, -1, -1, 1);
            }
            else {
                //Sum results
                vec3 dir = (llCorner + (x + 0.5) * right + (y + 0.5) * up).normalize(); //Center of each division of the pixel
                total = total + raytrace(eye->position, dir, 0, -1, -1, 1);
            }
    }

    //Color is the average of all the colors coming through the pixel
    result = (1 / divisionAmount)*total;
#endif
